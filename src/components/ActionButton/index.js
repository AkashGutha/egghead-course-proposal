...
class ActionButton extends React.Component {
  ...
  render() {
    ...
    const buttonClasses = `dib tc h-100 dark-gray no-underline pointer hover-bg-white ${noBorder ? '' : 'bl b--gray-secondary'} ${css.actionButton}`
    const confMsg = !confirmText
      ? `Are you sure you want to ${label.toLowerCase()} "${item}"?`
      : confirmText

    const okLabel = label && itemType && !customConfirm
      ? `Yes, ${label} ${itemType}`
      : 'OK'
    ...
    const ButtonBody = () => (
      <div className='flex items-center justify-center pv3 ph4'
        ...
        <span className='f6 dark-blue ml2'>
          {label}
        </span>
      </div>
    )
...
export const NewLessonButton = ({url, onClick, item = 'item', itemType}) => (
  <a href={url}
    className={`dib tc h-100 dark-gray no-underline pointer hover-bg-white bl b--gray-secondary ${css.actionButton}`}>
    <div className='flex items-center justify-center pv3 ph4' >
      <div className="mr1">
