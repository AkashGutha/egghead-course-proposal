...
const CardListItem = ({item, extraClasses}) => {
  ...
  const itemClasses = classNames('flex items-start relative dark-blue f6 lh-solid pointer pv3 pl4 pr3 bg-animate hover-bg-white no-underline ',
    css['card-progress-list-item'])
  const textClasses = classNames('ml2 flex justify-between flex-grow-1 lh-copy overflow-hidden', css['lesson-title'])
  ...
  return (
    <a href={href} className={`${itemClasses} ${extraClasses} ${watched ? watchedClasses : ''} ${current ? currentClasses : ''}`}>
      <img src={icon_url} className='mt1 h1' alt='' />
      <div className={`${textClasses} ${watched ? watchedTitleClasses : ''}`}>
        <div style={{fontSize: '12px'}}>
          {title}
        </div>
        <div className='ml3 tr o-60' style={{fontSize: '12px'}}>{length}</div>
      </div>
    </a>
  )
}
...
class CardList extends React.Component {
  ...
  componentDidMount() {
    ...
    if(playlist.lessons) {
      ...
      http.get(playlist.lessons_url).then(({data}) => this.setState({lessons: data}))
    }
  }
  render() {
    const {playlist, height} = this.props
    const containerStyle = {'width': '280px', 'height': `${height ? height : '384px'}`}
    const cardListShadow = classNames(css.cardListShadow)
    return (
      <div className={`bg-white flex flex-grow-1 justify-between flex-column ${cardListShadow}`} style={containerStyle}>
        <div className='bg-white-secondary self-stretch h-100 br2 overflow-y-scroll'>
          {this.state.lessons && (
            <ul className={classNames('list pa0 ma0 overflow-hidden', css['card-progress-list'])}>
              {
