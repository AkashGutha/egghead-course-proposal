...
const ColorLabel = ({text, color}) => {
  ...
  return (
    <span className={`f6 b no-underline br-pill mb2 mb0-ns ph3 pv2 dib ttu dib ${colorClasses}`} href='#'>
      {text}
    </span>
  )
}

const LessonStateButton = ({state, showIcon = false, showLabel = false,
                            showDescription = true, url, onClick}) => {
  if (isUndefined(detailsByLessonState[state])) {
    return <div />
  }

  return (
    <a href={url} onClick={onClick} className={`no-underline f6 mt2 ${css.lessonStateButton}`}>
