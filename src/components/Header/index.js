...
const ProfileDropdown = ({user_profile_url, name, username, email, avatar_url, id, is_pro, isDropdownOpened, toggleHandler, mouseOverHandler, mouseLeaveHandler}) => {
  ...
  return (
    ...
    >
      <button className={`${css.dropdownTrigger} ${openedClass}`} onClick={toggleHandler}>
...
const NavigationDropdown = ({navigationLinks, browseLinks, isDropdownOpened, toggleHandler, mouseOverHandler, mouseLeaveHandler, directionArrow}) => {
  const closedClass = isDropdownOpened ? '' : css.dropdownClosed
  const openedClass = isDropdownOpened ? css.dropdownOpened : ''
  return (
    <div
    ...
    >
      ...
      <div className={`${css.dropdownContentHolder} ${closedClass}`}>
        <ul className={css.dropdownLinksList}>
          {navigationLinks.map((item, k) => <NavigationLink item={item} key={k} />)}
        </ul>
        <div className={css.dropdownSeparator} />
        <ul className={css.dropdownLinksList}>
...
const ProfileHeaderItems = ({user_profile_url, production_board_path, path, name, username, email, id, is_pro, avatar_url, isProfileDropdownOpened, profileDropdownToggleHandler, mouseOverHandler, mouseLeaveHandler}) => {
  return (
    <div className={css.profileHeaderItemsHolder}>
      { !is_pro && !production_board_path ? <GoProButton location={path} /> : null }
      { production_board_path ? <InstructorButton location={path} path={production_board_path} /> : null }
      { id ?
        null :
        <ExternalTrackingLink
          href='/users/sign_in'
        ...
        <div>
          {
            id ?
              ...
              <ExternalTrackingLink
                href='/users/sign_up'
                track='clicked header signup'
                trackParams={{
                  location: path
                }}
                className={css.link}
              >
                <Button color='orange' outline size='medium'>Sign Up Free</Button>
              </ExternalTrackingLink>
          }
        </div>
      </div>
...
const MobileMenuIcon = ({handler, mobileMenuOpened}) => {
  ...
  return (
    <div className={`${css.mobileMenuTrigger} ${openedClass}`} onClick={handler}>
...
class Header extends React.Component {
  ...
  handleScrollBehavior = () => {
    if(window.pageYOffset > headerHideOffset && window.pageYOffset > this.state.currentScrollTop) {
      this.setState({
        headerIsHidden: true,
        currentScrollTop: window.pageYOffset
      })
    }
    else if(window.pageYOffset < this.state.currentScrollTop - 40 || window.pageYOffset === 0) {
      this.setState({
        headerIsHidden: false,
        currentScrollTop: window.pageYOffset
      })
  ...
  navigationDropdownToggleHandler = () => {
    this.setState({ isNavigationDropdownOpened: !this.state.isNavigationDropdownOpened })
  }

  navigationDropdownOpenHandler = () => {
    if (!this.isTouchDevice()) {
      this.setState({ isNavigationDropdownOpened: true })
    }
  }
  
  navigationDropdownCloseHandler = () => {
    if (!this.isTouchDevice()) {
  ...
  render() {
    const currentUser = get(this.props, 'currentUserStore.currentUser', this.props.current_user)
    const {
      user_profile_url,
      path,
      name,
    ...
    const { isNavigationDropdownOpened, isProfileDropdownOpened, mobileMenuOpened, headerIsHidden, isMobileViewport } = this.state
    ...
    `
    const navigationDropdownLinks = path === "/review" ? reviewLinks : navigationLinks
              ...
              <div className={css.headerContentInner}>
                <div className='flex items-center'>
                  ...
                  <Maybe condition={!isMobileViewport}>
                    <WatchClickOutside onClickOutside={this.navigationDropdownClickOutsideHandler}>
                      <NavigationDropdown
                        path={path}
                        navigationLinks={navigationDropdownLinks}
                        browseLinks={browseDropdownLinks}
                        isDropdownOpened={isNavigationDropdownOpened}
                        toggleHandler={this.navigationDropdownToggleHandler}
                        mouseOverHandler={this.navigationDropdownOpenHandler}
                        mouseLeaveHandler={this.navigationDropdownCloseHandler}
                      />
                    </WatchClickOutside>
                  </Maybe>
                  <HeaderLinks
                ...
                <WatchClickOutside onClickOutside={this.profileDropdownClickOutsideHandler}>
                  <ProfileHeaderItems
                    {...currentUser}
                    isProfileDropdownOpened={isProfileDropdownOpened}
                    profileDropdownToggleHandler={this.profileDropdownToggleHandler}
