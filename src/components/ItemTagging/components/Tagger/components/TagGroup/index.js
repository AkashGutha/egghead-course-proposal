...
@observer class TagGroup extends React.Component {
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.tagsInContext.length !== this.props.tagsInContext.length){
      const initiallyDisplayedTags = nextProps.tagsInContext.slice(0, this.props.truncateNumber)
  ...
  setShowMore = (value) => {
    if(value){
      ...
      const initiallyDisplayedTags = this.props.tagsInContext.slice(0, this.props.truncateNumber)
      this.setDisplayedTags(initiallyDisplayedTags)
    }
    this.showMore = value
  }
