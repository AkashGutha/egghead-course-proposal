...
class LessonUploadButton extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      uploadStatus: false,
      title_url: `${props.lesson.slug}-${uuid.generate()}`.replace(/[^\w\d_\-.]+/ig, ''),
    }
  }
  ...
  render() {
    ...
    const buttonClasses = `
      ...
      f6 
      pa2 
      ${uploadStatus ? styleForStatus(uploadStatus) : 'pointer bg-blue hover-bg-dark-blue'}
    `
        ...
        <div>
          <div className={inputContainerClasses} >
            <div className="flex">
              ...
              <span>{ labels[uploadStatus] || label || `${wistia_id ? 'Replace' : 'Upload'} Lesson Video` }</span>
            </div>
