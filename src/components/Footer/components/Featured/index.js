...
class Featured extends React.Component {
  ...
  render() {
    ...
    const hasEnoughContent = featuredCourses.length >= MIN_COURSES && featuredLessons.length >= MIN_LESSONS

    return (
      <div>{hasEnoughContent ? (
        <div>
          <div className='flex flex-column pt4 pt5-ns pb2 pb3-ns'>
            <div className='center ph2 ph3-ns eh-mw9'>
              <h2 className='f2 avenir fw5 white tc mt0 mb4 mb5-ns'>Featured</h2>
              <CardGrid2121 courses={featuredCourses} lessons={featuredLessons}/>
            </div>
          </div>
        </div>
      ) : (
