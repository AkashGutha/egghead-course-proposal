        ...
        {items.map((item, k) => {
          return (
              <div className='mr4' key={k}>
                <h5 className='avenir f6 f5-ns ttu tracked gray fw6 mt0 mb2 mb3-ns'>{item.header}</h5>
                  ...
                  {item.items.map((i, k) => {
                    ...
                    return (
                        <li className='pv1' key={k}>
                          <a href={i.link}
                             className={`db lh-copy hover-white no-underline grow ${path === i.link ? 'white' : 'white-50'}`}
                          >
                            {i.title}
                          </a>
                        </li>
...
class Footer extends React.Component {
  ...
  render() {
    const {path, name, username, email, id, is_pro, avatarUrl = 'http://placehold.it/150x150'} = this.props
    const footerDefaultClasses = 'pb2'
    const socialClasses = 'tc br2 f5 no-underline db-ns pa2 w2 h2 white grow-large'
        ...
        <div className='flex flex-column bg-base-secondary'>
          ...
          <footer className={footerDefaultClasses}>
            <div className='center ph3 ph4-ns pv4 pv5-ns eh-mw9'>
              ...
              <div
                  className='pt3 pb4 flex flex-column flex-row-l justify-between items-start bb b--light-navy'>
                <SearchSection />
              ...
              <div className='pt3 mt2 flex flex-column flex-row-ns justify-between items-center f6'>
                <ul className='list pa0 ma0 flex gray'>
                  <li>
                    <a href='/privacy' className='link no-underline gray hover-white'>Terms & Conditions</a>
                  </li>
                  <li className='ml4'>
                    &copy; egghead.io
                  </li>
